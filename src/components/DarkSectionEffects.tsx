  ...
  ({ variant = 'default' }, ref) => {
    ...
    return (
      <div ref={ref}>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[400px] rounded-full pointer-events-none pulse-glow-effect" style={{ background: `radial-gradient(circle, hsl(38 45% 55% / ${fx}), transparent 60%)` }} />
        <div className="absolute top-[30%] left-0 right-0 h-px pointer-events-none overflow-hidden">
          <div className="w-[200px] h-full shimmer-effect" style={{ background: 'linear-gradient(90deg, transparent, hsl(38 45% 55% / 0.1), transparent)' }} />
        </div>
        {[...Array(variant === 'hero' ? 5 : 3)].map((_, i) => (
          <motion.div key={i} className="absolute w-[2px] h-[2px] rounded-full pointer-events-none" style={{ background: 'hsl(38 45% 55%)', left: `${15 + i * 18}%`, top: `${20 + (i * 15) % 60}%` }}
        ...
        {variant !== 'cta' && (
          <div className="absolute top-0 right-[20%] bottom-0 w-px pointer-events-none overflow-hidden">
            <motion.div className="w-full h-[100px]" style={{ background: 'linear-gradient(180deg, transparent, hsl(38 45% 55% / 0.06), transparent)' }} animate={{ y: ['-100px', '500px'] }} transition={{ duration: 6, repeat: Infinity, ease: 'linear', delay: 1 }} />
          </div>
        )}
        <div className="absolute inset-0 pointer-events-none opacity-[0.01]" style={{ backgroundImage: 'repeating-linear-gradient(135deg, transparent, transparent 40px, hsl(38 45% 55%) 40px, hsl(38 45% 55%) 40.5px)' }} />
      </div>
    );
  }
);
